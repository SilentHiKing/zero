...
public class RecyclerTabLayout extends RecyclerView {
    ...
    private void getAttributes(Context context, AttributeSet attrs, int defStyle) {
        TypedArray a = context.obtainStyledAttributes(attrs, R.styleable.RecyclerTabLayout,
                defStyle, R.style.BaseRecyclerTabLayout);
        setIndicatorColor(a.getColor(R.styleable
                .RecyclerTabLayout_rtl_tabIndicatorColor, 0));
        setIndicatorHeight(a.getDimensionPixelSize(R.styleable
                .RecyclerTabLayout_rtl_tabIndicatorHeight, 0));

        mTabTextAppearance = a.getResourceId(R.styleable.RecyclerTabLayout_rtl_tabTextAppearance,
                R.style.BaseRecyclerTabLayout_Tab);

        mTabPaddingStart = mTabPaddingTop = mTabPaddingEnd = mTabPaddingBottom = a
                .getDimensionPixelSize(R.styleable.RecyclerTabLayout_rtl_tabPadding, 0);
        mTabPaddingStart = a.getDimensionPixelSize(
                R.styleable.RecyclerTabLayout_rtl_tabPaddingStart, mTabPaddingStart);
        mTabPaddingTop = a.getDimensionPixelSize(
                R.styleable.RecyclerTabLayout_rtl_tabPaddingTop, mTabPaddingTop);
        mTabPaddingEnd = a.getDimensionPixelSize(
                R.styleable.RecyclerTabLayout_rtl_tabPaddingEnd, mTabPaddingEnd);
        mTabPaddingBottom = a.getDimensionPixelSize(
                R.styleable.RecyclerTabLayout_rtl_tabPaddingBottom, mTabPaddingBottom);

        if (a.hasValue(R.styleable.RecyclerTabLayout_rtl_tabSelectedTextColor)) {
            mTabSelectedTextColor = a
                    .getColor(R.styleable.RecyclerTabLayout_rtl_tabSelectedTextColor, 0);
        ...
        if (mTabOnScreenLimit == 0) {
            mTabMinWidth = a.getDimensionPixelSize(
                    R.styleable.RecyclerTabLayout_rtl_tabMinWidth, 0);
            mTabMaxWidth = a.getDimensionPixelSize(
                    R.styleable.RecyclerTabLayout_rtl_tabMaxWidth, 0);
        }

        mTabBackgroundResId = a
                .getResourceId(R.styleable.RecyclerTabLayout_rtl_tabBackground, 0);
        mScrollEanbled = a.getBoolean(R.styleable.RecyclerTabLayout_rtl_scrollEnabled, true);
        a.recycle();
    }
    ...
    public void onDraw(Canvas canvas) {
        ...
        int right;
        if (isLayoutRtl()) {
            left = view.getLeft() + view.getPaddingLeft() - mIndicatorScroll - mIndicatorGap;
            right = view.getRight() - view.getPaddingRight() - mIndicatorScroll + mIndicatorGap;
        } else {
            left = view.getLeft() + view.getPaddingLeft() + mIndicatorScroll - mIndicatorGap;
            right = view.getRight() - view.getPaddingRight() + mIndicatorScroll + mIndicatorGap;
           /* left = view.getLeft() + mIndicatorScroll;
            right = view.getRight() + mIndicatorScroll;*/
        }
    ...
    protected static class ViewPagerOnPageChangeListener implements ViewPager.OnPageChangeListener {
        ...
        public void onPageScrolled(int position, float positionOffset, int positionOffsetPixels) {
            Log.d("RecyclerTabLayout", "positionOffset = " + positionOffset + "position = " + position);
